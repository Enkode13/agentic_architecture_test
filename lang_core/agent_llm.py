...
class LLM:
    ...
    def agent_llm(self, current_state):
        data = current_state["scratchpad"]
        scratchpad = "\n".join([f"Tool_name: {d['Tool_name:']}, Tool Result: {d['Tool Result:']}" for d in data])
        user_prompt = f"""
        ...
        system_prompt = """
            ...
            4. "scratchpad" contains prior tools' name and results, and must be used to avoid repeating work.
            5. Stop only when the full task is completely solved.
